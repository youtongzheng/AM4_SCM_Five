...
!yzheng: prepare driver inputs for FIVE arrays
    if (do_five) then
      ...
      write (*,*) 'Rad_flux(1)%block(blk)%tdt_lw', Rad_flux(1)%block(1)%tdt_lw
      write (*,*) 'Rad_flux_five(1)%block(blk)%tdt_lw', Rad_flux_five(1)%block(1)%tdt_lw
      write (*,*) 'Rad_flux(1)%block(blk)%tdt_rad', Rad_flux(1)%block(1)%tdt_rad
      write (*,*) 'Rad_flux_five(1)%block(blk)%tdt_rad', Rad_flux_five(1)%block(1)%tdt_rad
      write (*,*) 'Rad_flux(1)%block(blk)%extinction', Rad_flux(1)%block(1)%extinction
      write (*,*) 'Rad_flux_five(1)%block(blk)%extinction', Rad_flux_five(1)%block(1)%extinction

      write (*,*) 'Rad_flux(1)%block(blk)%flux_sw_down_vis_dir', Rad_flux(1)%block(1)%flux_sw_down_vis_dir
      write (*,*) 'Rad_flux_five(1)%block(blk)%flux_sw_down_vis_dir', Rad_flux_five(1)%block(1)%flux_sw_down_vis_dir
    endif
...
!--- indices for one-based arrays
       ...
       if (.not. do_five) then
        ...
        write (*,*) 'Physics down sphum', Physics%block(blk)%q(:,:,:,sphum)
        write (*,*) 'Physics_tendency down sphum', Physics_tendency%block(blk)%q_dt(:,:,:,sphum)

        write (*,*) 'Physics down nqa', Physics%block(blk)%q(:,:,:,nqa)
        write (*,*) 'Physics_tendency down nqa', Physics_tendency%block(blk)%q_dt(:,:,:,nqa)

        write (*,*) 'Surf_diff down delta_tr', Atmos%Surf_diff%delta_tr
        write (*,*) 'Surf_diff down dflux_tr', Atmos%Surf_diff%dflux_tr
        write (*,*) 'Surf_diff down dtmass', Atmos%Surf_diff%dtmass
        write (*,*) 'Surf_diff down delta_u', Atmos%Surf_diff%delta_u
        write (*,*) 'Surf_diff down delta_v', Atmos%Surf_diff%delta_v
    enddo
...
!--- indices for one-based arrays
    ...
    write (*,*) 'Physics up sphum', Physics%block(blk)%q(:,:,:,sphum)
    write (*,*) 'Physics_tendency up sphum', Physics_tendency%block(blk)%q_dt(:,:,:,sphum)

    write (*,*) 'Physics up nqa', Physics%block(blk)%q(:,:,:,nqa)
    write (*,*) 'Physics_tendency up nqa', Physics_tendency%block(blk)%q_dt(:,:,:,nqa)
