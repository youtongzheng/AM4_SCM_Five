...
!
!       WFC: 2014/07/30, modified to run for MC3E SCM intercomparison
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!
   ...
   use            fms_mod, only:  mpp_pe, FATAL,   &
                                  ...
                                  field_exist, file_exist, error_mesg, &
...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public mc3e_data_read, mc3e_forc_init, mc3e_forc_end, update_mc3e_forc, &
          mc3e_forc_diagnostic_init,                      &
          mc3e_surface_flux_loop, get_mc3e_sfc, get_mc3e_sst
...
!
!            mc3e_data_read     reads forcing data files and initializes
!                              any needed parameters of the run
!            mc3e_forc_init     initializes prognostic variables:           
!                              T,u,v,qv,ql,qi
!            mc3e_forc_end      deallocates allocated spaces
!            update_mc3e_forc   a call to this routine returns the 
...
!           N               nudging only
!           S          semi-prognistic. Update T and Q to forcing quantities.
!
...
REAL                           :: sphum_multiplier = 1.0 
! Multiplier for launch level (currently k=2) of donner_deep moisture.
! Use to investigate if localised dry spot is affecting convection.

!--------------------- version number ----------------------------------
!
        
character(len=128) :: Version = '$Id: scm_mc3e.F90,v 1.1.2.1.2.2 2014/08/18 15:16:54 wfc Exp $'
character(len=128) :: Tagname = '$Name: a1_closure_wfc $'
        

NAMELIST /SCM_mc3e_NML/  do_netcdf_restart,                             &
                        ...
                        use_given_omega_surface, sphum_multiplier, a1_precfrac

LOGICAL :: mc3e_forc_initialized = .FALSE.
...
CHARACTER(len=64)                         :: fname_nc='INPUT/mc3e_forc.res.nc'
...
!      ----- read namelist -----
      ...
      ! set mc3e_forc_initialized .TRUE.

      if (mc3e_forc_initialized) return
      mc3e_forc_initialized = .TRUE.


      if (file_exist('input.nml')) then
#ifdef INTERNAL_FILE_NML
        read (input_nml_file, nml=SCM_mc3e_NML, iostat=io)
        ierr = check_nml_error(io, 'SCM_mc3e_NML')
#else
        unit = open_namelist_file ()
        io=1; do while (io .ne. 0)
          read  (unit, nml=SCM_mc3e_NML, iostat=io, end=10)
        enddo
...
!--------- write version and namelist to standard log ------------
        ...
        if ( mpp_pe() == mpp_root_pe() ) &
          write ( logunit, nml=SCM_mc3e_NML )
      ...
      !dummy check -------         
       if (temp_vert_advec_scheme .gt. 6 ) then
            ...
            call error_mesg ( 'mc3e_data_read in scm_mc3e_mod',         &
       ...
       if( file_exist(trim(forcing_nc)) ) then
         ...
         ivert= siz(1)
         ! For MC3E, assume the number of fields is 43.
         nfields = 43
         ! For MC3E, we assume the number of layer fields is 19 (needed for arscl variable).
         nlayer_fields = 19
         ASCII_VARA = .FALSE.
       endif
...
!            if (i.eq.46) alb_day(:)=vardat(1,1:itime)
         ...
         do t = 1,itime
           ...
           call read_data(forcing_nc, 'T', t_forc(1:ivert,t)        ,no_domain=.true., timelevel=t)
           ...
           if (field_exist(forcing_nc,'cld')) then
...
!wfc
       ...
       ! Use sphum_multiplier to investigate dry locales in forcing data.
!         q_forc(2,:) = q_forc(2,:) * sphum_multiplier
       endif  
...
!
    ...
    else if (file_exist('INPUT/mc3e_forc.res')) then

          unit=open_restart_file('INPUT/mc3e_forc.res',action='read')
...
subroutine mc3e_forc_init(time_interp, pdamp, As, Bs)
#include "fv_arrays.h"
...
!-----------------------------------------------------------------------
 ...
 end subroutine mc3e_forc_init

!#######################################################################
!#######################################################################


subroutine mc3e_forc_end ()

  integer :: unit
  character(len=64) :: fname_nc='RESTART/mc3e_forc.res.nc'
  
  if (.NOT.mc3e_forc_initialized) return
  mc3e_forc_initialized = .FALSE.
...
! write to the restart file
  ...
  if( do_netcdf_restart ) then
    ...
    if (mpp_pe() == mpp_root_pe()) then
       ...
       unit=open_restart_file('RESTART/mc3e_forc.res',action='write')
...
end subroutine mc3e_forc_end


!#######################################################################
!#######################################################################

subroutine mc3e_forc_diagnostic_init(axes, Time)
...
end subroutine mc3e_forc_diagnostic_init
...
subroutine update_mc3e_forc(time_interp,time_diag,dt_int,pdamp )
#include "fv_arrays.h"
...
if (time_interp < time_layer(1)) then
    ...
    call error_mesg ( 'update_mc3e_forc in scm_mc3e_mod',                &
...
case("S")
 ...
 case default
        call error_mesg ( 'update_mc3e_forc in scm_gate_mod',            &
                 'Bad forcing method : '//FORCMETHOD, FATAL )
end select
...
end subroutine update_mc3e_forc
...
subroutine mc3e_surface_flux_loop (                                     &
...
!                         ((flux_q(k)-EVAPFLUX) / max(dedq_surf(k),1.e-10))))
   ...
   if ( nouter .gt. niter_max .or. ninner .gt. niter_max ) then
     ...
     do k = 1, npoints
       ...
       if ( hlv*abs(flux_q(k)- EVAPFLUX) .gt. max_flux_error .or.&
         ...
         !Avoid a divide by 0 if flux_u=flux_v=0
   ...
   if (mpp_pe() == mpp_root_pe()) then
     write(*,'(A40,2F10.4)') 'desired fluxes  (sensible, latent): ',     &
          SENFLUX, hlv*EVAPFLUX
     write(*,'(A40,2F10.4)') 'iterated fluxes (sensible, latent): ',     &
          flux_t, hlv*flux_q
     write(*,'(A40,2F10.4)') 'iterated fluxes (t_surf    1.2*q_atm): ',  &
          t_surf, 1.2*q_atm
     write(*,'(A40,2F10.4)') 'iterated fluxes (t_ca_tmp, q_surf_tmp): ', &
          t_ca_tmp, q_surf_tmp
   end if
...
end subroutine mc3e_surface_flux_loop

!#######################################################################
!########################################################################
! This subroutine returns surface conditions

subroutine get_mc3e_sfc( albedo_obs_out, tskin_out,     &
...
end subroutine get_mc3e_sfc

!########################################################################
! This subroutine returns imposed SST

subroutine get_mc3e_sst( sst )
