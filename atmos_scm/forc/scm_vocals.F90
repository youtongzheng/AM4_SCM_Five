module scm_vocals_mod
 
 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!       VOCALS (Oct.-Nov., 2008) FORCING MODULE
!
!       Dec 2011
...
!
   ...
   public vocals_data_read, vocals_forc_init, vocals_forc_end, update_vocals_forc, &
          vocals_forc_diagnostic_init,        get_vocals_flx
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!        The module contains the following subroutines:
!
!            vocals_data_read      reads forcing data files and initializes
!                                 any needed parameters of the run
!            vocals_forc_init      initializes prognostic variables:           
!                                 T,u,v,qv,ql,qi
!            vocals_forc_end       deallocates allocated spaces
!            update_vocals_forc    a call to this routine returns the 
...
character(len=8) :: mod_name = 'vocals'
...
character*64                   :: fname_forc='INPUT/19810101.atmos_col_003.nc'

LOGICAL, PRIVATE               :: vocals_forc_initialized = .FALSE.

!--------------------- version number ----------------------------------
!
character(len=128) :: Version = '$Id: scm_vocals.F90,v 1.1.2.1 2014/02/10 18:17:23 wfc Exp $'
character(len=128) :: Tagname = '$Name: new_gate_wfc $'
integer, dimension(1) :: restart_versions = (/ 1 /)
        
NAMELIST /scm_vocals_nml/   fname_forc, forc_begin_date,  dt_forc,                      &
...
integer                        :: i_forc_time
       ...
       ! set vocals_forc_initialized .TRUE.

       if ( vocals_forc_initialized ) return
       vocals_forc_initialized = .TRUE.

!    --------- read namelist --------
       if (file_exist('input.nml')) then
            unit = open_namelist_file()
            ierr=1; do while (ierr /= 0)
            read  (unit, nml=scm_vocals_nml, iostat=io, end=10)
            ierr = check_nml_error(io,'scm_vocals_nml')
            enddo
10          call close_file (unit)
 ...
 !--------- write version number and namelist --------
       ...
       if(mpp_pe() == mpp_root_pe() ) write(stdlog(),nml=scm_vocals_nml)


       if ( file_exist( fname_forc ) ) then
          if(mpp_pe() == mpp_root_pe() ) call mpp_error ('scm_vocals_mod', &
...
!      
       ...
       endif     !file_exist('INPUT/*.atmos_col_003.nc' )
!-----------------------------------------------------------------------
                            
end subroutine vocals_data_read


!#######################################################################
subroutine vocals_forc_init(time_interp, As, Bs)
#include "fv_arrays.h"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
! SCM version of reference pressure at full and half levels
    !   do k = 1,KDIM+1
    !        ph(k)= As(k)+ p00*Bs(k)*100.0
    !   enddo

    !   if( ph(1) <= epsilon(ph) ) then
    !     lph(1)  = 0.0
     !    lph(2:) = log( ph(2:) )
     !  else
     !    lph= log( ph )
...
! --- Create full pressure levels (mb)
   !    do k = 1,KDIM
   !         pf(k) = (ph(k+1)*lph(k+1) - ph(k)*lph(k)) / (ph(k+1)-ph(k)) - 1.0
   !         pf(k) = exp(pf(k)) * 0.01
   !    enddo
      ...
      field_int_more(1:n_forc_levels) = qv_profile(1,1, 1:n_forc_levels, itime_more) &
       ...
       q(:,:,k,nql)= 0.0
       q(:,:,k,nqi)= 0.0
       if ( nqn > 0 )  q(:,:,k,nqn) = 0.0
       if ( nqni > 0 ) q(:,:,k,nqni)= 0.0
       ...
       enddo
!-----------------------------------------------------------------------
 end subroutine vocals_forc_init



!#######################################################################
subroutine vocals_forc_end()

  if (.NOT.vocals_forc_initialized) return
  vocals_forc_initialized = .FALSE.

end subroutine vocals_forc_end

!#######################################################################


subroutine vocals_forc_diagnostic_init(axes, Time)
...
!-----------------------------------------------------------------------
end subroutine vocals_forc_diagnostic_init


!#######################################################################
subroutine update_vocals_forc(time_interp,time_diag,dt_int)
#include "fv_arrays.h"
...
! ------------------------------------------------------------------------------------------------------------
       ...
       if ( do_nudging ) then
         ...
         call interp_2d_field( pf(1:KDIM), weight_less, weight_more, plev_forc(1:n_forc_levels),    &
              field_int_less(1:n_forc_levels), field_int_more (1:n_forc_levels), field_interp_out(1:KDIM) )
         do k = 1, KDIM
            u_dt(:,:,k) = - ( ua(:,:,k) - field_interp_out(k) ) / tao_nudge
         enddo
         ...
         call interp_2d_field( pf(1:KDIM), weight_less, weight_more, plev_forc(1:n_forc_levels),    &
              field_int_less(1:n_forc_levels), field_int_more (1:n_forc_levels), field_interp_out(1:KDIM) )
         do k = 1, KDIM
            v_dt(:,:,k) = - ( va(:,:,k) - field_interp_out(k) ) / tao_nudge
         enddo
...
! --- compute geostrophic tendencies
       ...
       if ( id_tdt_dyn > 0 )  &
             used = send_data(  id_tdt_dyn, t_dt(:,:,:), Time_diag, 1, 1 )

       if ( id_udt_dyn > 0 )  &
             used = send_data(  id_udt_dyn, u_dt(:,:,:), time_diag, 1, 1 )

       if ( id_vdt_dyn > 0 )  &
             used = send_data(  id_vdt_dyn, v_dt(:,:,:), time_diag, 1, 1 )

       if ( id_qadt_dyn > 0 ) &
             used = send_data(  id_qadt_dyn, q_dt(:,:,:,nqa), time_diag, 1, 1 )

       if ( id_qdt_dyn > 0 )  &
             used = send_data(  id_qdt_dyn, q_dt(:,:,:,nsphum), time_diag, 1, 1 )

       if ( id_qldt_dyn > 0 ) &
             used = send_data(  id_qldt_dyn, q_dt(:,:,:,nql), time_diag, 1, 1 )

       if ( id_qidt_dyn > 0 ) &
             used = send_data(  id_qidt_dyn, q_dt(:,:,:,nqi), time_diag, 1, 1 )

       if( nqn > 0 .and. id_qndt_dyn > 0)   &
             used = send_data(  id_qndt_dyn,  q_dt(:,:,:,nqn), time_diag, 1, 1 )

       if( nqni > 0 .and. id_qnidt_dyn > 0) &
             used = send_data(  id_qnidt_dyn, q_dt(:,:,:,nqni), time_diag, 1, 1 )


! the surface fluxes are called before update_vocals_forc, 
! so the next time index is calculated for the surface fluxes at next time step!
       ...
       print*, ' update_forc, itime_more = ', itime_more,  weight_more
!-----------------------------------------------------------------------
end subroutine update_vocals_forc
...
subroutine get_vocals_flx( ustar, flux_t, flux_q )
