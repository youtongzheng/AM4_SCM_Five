...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public atex_data_read, atex_forc_init, atex_forc_end, update_atex_forc, &
          atex_forc_diagnostic_init, add_atex_tdtlw, add_atex_tdtsw, atex_sfclyr

character(len=8) :: mod_name = 'scm_atex'
character(len=7) :: mod_name_diag = 'forcing'

real, public      :: psfc = 1015.e2
real, public      :: zsfc = 0.0
...
namelist /scm_atex_nml/ tracer_vert_advec_scheme,   &
...
subroutine atex_data_read(kmax)
...
character*64              :: fname_res='INPUT/atex.res.nc'
type(time_type)           :: Time_init
     
integer :: year, month, day, logunit
...
!-------- read namelist --------
#ifdef INTERNAL_FILE_NML
   READ (input_nml_file, nml=scm_atex_nml, iostat=io)
   ierr = check_nml_error(io, 'scm_atex_nml')
#else
   if (file_exist('input.nml')) then
      unit = open_namelist_file()
      ierr=1; do while (ierr /= 0)
         read  (unit, nml=scm_atex_nml, iostat=io, end=10)
         ierr = check_nml_error(io,'scm_atex_nml')
      enddo
...
!--------- write version number and namelist --------
   ...
   if(mpp_pe() == mpp_root_pe() ) then
     logunit =stdlog()
     write(logunit,nml=scm_atex_nml)
   endif
...
! ---> h1g, 2010-10-08
   if (file_exist('INPUT/atex.res.nc') ) then
      if(mpp_pe() == mpp_root_pe() ) call mpp_error ('scm_atex_mod', &
         'Reading netCDF formatted restart file: INPUT/atex.res.nc', NOTE)
      if( allocated(u_geos) ) call read_data (fname_res, 'u_geos',  u_geos)
      if( allocated(v_geos) ) call read_data (fname_res, 'v_geos',  v_geos)
   endif
! <--- h1g, 2010-10-08
        
end subroutine atex_data_read

!#######################################################################
! Subroutine to initialize case forcings

subroutine atex_forc_init(time_interp,As,Bs,elev)
#include "fv_arrays.h"
...
real, dimension(size(pt,3))   :: u_atex, v_atex, ug_atex, vg_atex, T_atex, qv_atex
...
!   Iteration to compute atex sounding
    ...
    do while ( maxerror > 0.001 .and. i < itmax )
     ...
     do k=1,kdim
      ...
      call atex_snd( zf(1,1,k),                      & 
                     u_atex(k), v_atex(k), ug_atex(k), vg_atex(k),  &
                     T_atex(k), qv_atex(k) )
      ua(:,:,k) = u_atex(k)
    ...
    if ( i >= itmax ) then
      call error_mesg('atex_forc_init',  &
...
end subroutine atex_forc_init

!#######################################################################

subroutine atex_forc_end ()
character*64                 :: fname_res='RESTART/atex.res.nc'
...
! ---> h1g, 2010-10-08
     if( do_netcdf_restart ) then
           if (mpp_pe() == mpp_root_pe()) then
              call mpp_error ('scm_atex_mod', 'Writing netCDF formatted restart file: RESTART/atex.res.nc', NOTE)
           endif
...
! <--- h1g, 2010-10-08
 ...
 end subroutine atex_forc_end

!#######################################################################

subroutine atex_forc_diagnostic_init(axes, Time)
...
end subroutine atex_forc_diagnostic_init

!#######################################################################
! Subroutine to apply atex forcings

subroutine update_atex_forc(time_interp,time_diag,dt_int, elev)
#include "fv_arrays.h"
...
! initialization
   ...
   current_toal_sec=real(current_sec-current_sec0)+86400.0*real(current_days-current_days0)
...
if (  current_toal_sec >= 5400.0 ) then
      do k = 2, kdim
         if ( ( q(1,1, k,nsphum)/(1.0-q(1,1, k,nsphum) ) +q(1,1,k,nql) ) > 6.5e-3 ) then
!---> h1g, 2010-09-17 
           zi(1,1) = zf(1,1,k-1)
!<--- h1g, 2010-09-17 
           exit
   ...
   do k=1,kdim
      if ( zf(1,1,k) <= zi(1,1) ) then
         omga(:,:,k) = pf(1,1,k)/(rdgas*pt(1,1,k))*grav * 0.0065 * zf(1,1,k)/zi(1,1)
      elseif ( zf(1,1,k) <= zi(1,1) + 300.) then
          omga(:,:,k) = pf(1,1,k)/(rdgas*pt(1,1,k))*grav * 0.0065 &
	                          * (1.0- ( zf(1,1,k) - zi(1,1) )/300. )
      else
           omga(:,:,k) = 0.0
      endif
   end do
   ...
   omega_h = 0.0
   do k=2,kdim
      if ( zh(1,1,k) <= zi(1,1) ) then
         omega_h(:,:,k) = ph(1,1,k)/(rdgas*0.5*(pt(1,1,k)+pt(1,1,k-1)))*grav &
				 * 0.0065 * zh(1,1,k)/zi(1,1) 
      elseif (  zh(1,1,k) <= zi(1,1) + 300.) then
           omega_h(:,:,k) = ph(1,1,k)/(rdgas*0.5*(pt(1,1,k)+pt(1,1,k-1)))*grav &
                                    * 0.0065 * (1.0- ( zh(1,1,k) - zi(1,1) )/300. )
      else
         omega_h(:,:,k) = 0.
      end if
   end do
   ...
   do k=1, kdim
      if ( zf(1,1,k) <  zi(1,1) ) then
         dT_lf(1,1,k) =  -1.1575e-5 * ( 3. - zf(1,1,k)/zi (1,1) )
      elseif (  zh(1,1,k) <  zi(1,1) + 300.) then
          dT_lf(1,1,k) =  -2.315e-5 * (1.0- ( zf(1,1,k) - zi(1,1) )/300. )
      else
         dT_lf(1,1,k) = 0.0
      end if
   end do
   !convert from potential temperature tendency to (air) temperature tendency
   dT_lf = dT_lf * (pf/p00)**(rdgas/cp_air)


   dqv_lf = 0.0
   do k=1, kdim
      if ( zf(1,1,k) < zi(1,1) ) then
          dqv_lf(1,1,k) = -1.58e-8 * ( 1. - zf(1,1,k)/zi(1,1)  )    ! Brian
     else
         dqv_lf(1,1,k) = 0.0
      end if
   end do
  ...
  if (do_rad) then
     klwp = 0.0
     do k=2, kdim
      ! klwp(k)= klwp(k-1)+ ql(1,1,k) * dp(1,1,k)/grav
       klwp(k)= klwp(k-1)+ q(1,1,k-1,nql) * dp(1,1,k-1)/grav  !h1g to achieve CF=0.4
     end do
     frad(1,1,1:kdim)=74. * EXP( -130. * 1.0 * klwp(1:kdim) )
     frad(1,1,kdim+1)=frad(1,1,kdim)
     do k=1,kdim
   ...
   if (do_vadv) then
      ...
      END SELECT          
      call vert_advection(dts,omega_h,delp,pt,dT_vadv,scheme=vadvec_scheme,form=ADVECTIVE_FORM)
...
dv_geos = 0.0
if (do_geo) then
!   fcriolis=2.*2.*3.1415926/86400.*sin(Hgrid%Tmp%aph(1,1))
   fcriolis= 0.375e-4        ! for Atex
   do k=1, kdim
...
end subroutine add_atex_tdtsw


!########################################################################
! Subroutine retuns atex initial sounding

       subroutine atex_snd(  z, u, v, ug, vg, T, qv )
       implicit none
...
!      u wind
         if ( z < 150.0 ) then
           u = -11. + (-10.55 + 11.) / 150.0 * z 
         else if ( z <700.0 ) then
           u = -10.55 + ( -8.9 + 10.55 ) / ( 700.0 - 150.0) * (z - 150.0)
         else if ( z <750.0 ) then
           u = -8.9 + ( -8.75 + 8.9 ) / ( 750.0 - 700.0) * (z - 700.0)
         else if ( z < 1400.0 ) then
           u = -8.75 + ( -6.8 + 8.75 ) / ( 1400.0 - 750.0) * (z - 750.0)
	 else if ( z < 1650.0 ) then
	   u = -6.8 + ( -5.75 +6.8  ) / ( 1650.0 - 1400.0) * (z - 1400.0)
	 else if ( z < 4000.0 ) then
           u= -5.75 + ( 1. + 5.75 ) / ( 4000.0 - 1650.0) * (z - 1650.0)
	else
	   u =1.
	 end if
         ug = u
...
!      v wind
        if ( z < 150.0 ) then
           ...
           v = -1.9 + ( -1.1 + 1.9 ) / ( 700.0 - 150.0) * (z - 150.0)
         else if ( z <750.0 ) then
           v = -1.1 + ( -1. + 1.1 ) / ( 750.0 - 700.0) * (z - 700.0)
         else if ( z < 1400.0 ) then
           v = -1. + ( -0.14 + 1.) / ( 1400.0 - 750.0) * (z - 750.0)
	 else if ( z < 1650.0 ) then
	   v = -0.14 + ( 0.18 + 0.14 ) / ( 1650.0 - 1400.0) * (z - 1400.0)
	 else if ( z < 4000.0 ) then
           v= 0.18 + ( 2.75 - 0.18 ) / ( 4000.0 - 1650.0) * (z - 1650.0)
	else
	   v =2.75
	 end if
         vg = v
...
!      v wind
        if ( z < 150.0 ) then
           qv = 0.013 + ( 0.0125 - 0.013 ) / 150.0 * z 
         else if ( z <700.0 ) then
           qv = 0.0125 + ( 0.0125 - 0.0125 ) / ( 700.0 - 150.0) * (z - 150.0)
         else if ( z <750.0 ) then
           qv = 0.0125 + ( 0.0115 -0.0125 ) / ( 750.0 - 700.0) * (z - 700.0)
         else if ( z < 1400.0 ) then
           qv = 0.0115 + ( 0.01025 - 0.0115 ) / ( 1400.0 - 750.0) * (z - 750.0)
	 else if ( z < 1650.0 ) then
	   qv = 0.01025 + ( 0.0045 - 0.01025 ) / ( 1650.0 - 1400.0) * (z - 1400.0)
	 else if ( z < 4000.0 ) then
           qv= 0.0045 + ( 0.0045 - 0.0045 ) / ( 4000.0 - 1650.0) * (z - 1650.0)
	 elseif ( z < 9000.0 ) then
           qv = 0.0045 + (0. - 0.0045) / (9000.0 - 4000.0) * (z - 4000.0)
         else
           qv = 0.0	  
	 end if
...
!      Temperature
        if ( z < 150.0 ) then
           T = 297.0 + (  295.557 - 297.0 ) / 150.0 * z 
         else if ( z <700.0 ) then
           T = 295.557 + (  290.265 - 295.557 ) / ( 700.0 - 150.0) * (z - 150.0)
         else if ( z <750.0 ) then
           T = 290.265 + ( 290.111 - 290.265 ) / ( 750.0 - 700.0) * (z - 700.0)
         else if ( z < 1400.0 ) then
           T = 290.111 + ( 285.542- 290.111 ) / ( 1400.0 - 750.0) * (z - 750.0)
	 else if ( z < 1650.0 ) then
	   T = 285.542 + ( 291.541 -  285.542) / ( 1650.0 - 1400.0) * (z - 1400.0)
	 else if ( z < 4000.0 ) then
           T= 291.541 + ( 276.2 - 291.541 ) / ( 4000.0 - 1650.0) * (z - 1650.0)
...
! ---> h1g, 2010-09-17
! there are slight differences between temperature and potential temperature
! add some amount to stabilize the sounding
         T = T + 0.04
! <--- h1g, 2010-09-17

       return
       end subroutine atex_snd
