...
use  sat_vapor_pres_mod,only : sat_vapor_pres_init   ! ZNT 02/20/2020
   ...
   public dcbl_data_read, dcbl_forc_init, dcbl_forc_end, update_dcbl_forc, &
          dcbl_forc_diagnostic_init, get_dcbl_flx

character(len=9) :: mod_name = 'scm_dcbl'
...
real,    private               :: psfc = 1000.0e2
real,    private               :: zsfc =    0.0
real,    private               :: ustar_sfc   = 0.28
real,    private               :: wthl_sfc  = 0.0
real,    private               :: wqt_sfc   = 0.0
real,    private               :: dthldz    = 0.0
...
namelist /scm_dcbl_nml/ tracer_vert_advec_scheme,                   &
                        temp_vert_advec_scheme,                     &
                        momentum_vert_advec_scheme,                 &
                        vert_advec_cond,                            &
                        do_rad, do_geo, do_vadv,                    &
                        do_nudge,                                   &
                        ustar_sfc, wthl_sfc, wqt_sfc, dthldz,       &
                        configuration
...
subroutine dcbl_data_read(kmax)
...
character*64                              :: fname_res='INPUT/dcbl.res.nc'
...
!-------- read namelist --------
#ifdef INTERNAL_FILE_NML
   read (input_nml_file, nml=scm_dcbl_nml, iostat=io)
   ierr = check_nml_error(io, 'scm_dcbl_nml')
#else
   if (file_exist('input.nml')) then
      unit = open_namelist_file()
      ierr=1; do while (ierr /= 0)
         read  (unit, nml=scm_dcbl_nml, iostat=io, end=10)
         ierr = check_nml_error(io,'scm_dcbl_nml')
      enddo
...
!--------- write version number and namelist --------
   ...
   if(mpp_pe() == mpp_root_pe() ) then
     logunit =stdlog()
     write(logunit,nml=scm_dcbl_nml)
   endif
...
! ---> h1g, 2010-10-07
   if (file_exist('INPUT/dcbl.res.nc') ) then
      if(mpp_pe() == mpp_root_pe() ) call mpp_error ('scm_dcbl_mod', &
         'Reading netCDF formatted restart file: INPUT/dcbl.res.nc', NOTE)
      if( allocated(u_geos) ) call read_data (fname_res, 'u_geos',  u_geos)
      if( allocated(v_geos) ) call read_data (fname_res, 'v_geos',  v_geos)
   endif
! <--- h1g, 2010-10-07

end subroutine dcbl_data_read

!#######################################################################
! Subroutine to initialize case forcings

subroutine dcbl_forc_init(time_interp,As,Bs,elev)
#include "fv_arrays.h"
...
!   Iteration to compute DCBL sounding
    ...
    do while ( maxerror > 0.001 .and. i < itmax )
     ...
     do k=1,kdim
      ...
      call dcbl_snd( zf(1,1,k), u_snd, v_snd, ug_snd, vg_snd, thetal_snd, qt_snd )
      u_geos(:,:,k) = ug_snd
      v_geos(:,:,k) = vg_snd
    ...
    if ( i >= itmax ) then
      call error_mesg('dcbl_forc_init',  &
...
end subroutine dcbl_forc_init

!#######################################################################

subroutine dcbl_forc_end ()
character*64                 :: fname_res='RESTART/dcbl.res.nc'
...
! ---> h1g, 2010-10-07
     if( do_netcdf_restart ) then
           if (mpp_pe() == mpp_root_pe()) then
              call mpp_error ('scm_dcbl_mod', 'Writing netCDF formatted restart file: RESTART/dcbl.res.nc', NOTE)
           endif
...
end subroutine dcbl_forc_end

!#######################################################################

subroutine dcbl_forc_diagnostic_init(axes, Time)
...
end subroutine dcbl_forc_diagnostic_init

!#######################################################################
! Subroutine to apply DCBL forcings

subroutine update_dcbl_forc(time_interp,time_diag,dt_int,elev)
#include "fv_arrays.h"
...
end subroutine update_dcbl_forc

!########################################################################
! This subroutine returns imposed surface fluxes

subroutine get_dcbl_flx( ustar, wthl, wqt )

  implicit none
  real, intent(out), dimension(:) :: ustar, wthl, wqt

  ustar = ustar_sfc
  wthl = wthl_sfc
  wqt = wqt_sfc

end subroutine get_dcbl_flx

!########################################################################
! Subroutine retuns DCBL initial sounding

       subroutine dcbl_snd( z, u, v, ug, vg, thetal, qt )
       implicit none
