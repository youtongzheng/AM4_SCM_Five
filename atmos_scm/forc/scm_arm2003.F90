module scm_arm2003_mod 



! this subroutine is for the ARM 2003 aerosol IOP
! h1g
...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public arm2003_data_read, arm2003_forc_init, arm2003_forc_end, update_arm2003_forc, &
          SCM_arm2003_NML,  arm2003_forc_diagnostic_init,                      &
...
!
!            arm2003_data_read     reads forcing data files and initializes
!                              any needed parameters of the run
!            arm2003_forc_init     initializes prognostic variables:           
!                              T,u,v,qv,ql,qi
!            arm2003_forc_end      deallocates allocated spaces
!            update_arm2003_forc   a call to this routine returns the 
...
INTEGER, PRIVATE               :: ivert_stand = 35
...
character(len=128) :: Version = '$Id: scm_arm2003.F90,v 1.1.2.1.2.1 2014/08/18 15:12:30 wfc Exp $'
character(len=128) :: Tagname = '$Name: new_gate_wfc $'
        

NAMELIST /SCM_arm2003_NML/  do_netcdf_restart,                             &
...
LOGICAL :: arm2003_forc_initialized = .FALSE.
...
CHARACTER(len=64)                         :: fname_nc='INPUT/arm2003_forc.res.nc'
...
!      ----- read namelist -----
      ...
      ! set arm2003_forc_initialized .TRUE.

      if (arm2003_forc_initialized) return
      arm2003_forc_initialized = .TRUE.
      
#ifdef INTERNAL_FILE_NML
      read (input_nml_file, nml=SCM_arm2003_NML, iostat=io)
      ierr = check_nml_error(io, 'SCM_arm2003_NML')
#else
      if (file_exist('input.nml')) then
         unit = open_namelist_file ()
         io=1; do while (io .ne. 0)
            read  (unit, nml=SCM_arm2003_NML, iostat=io, end=10)
         enddo
  10     call close_file (unit)
      ...
      !dummy check -------         
       if (temp_vert_advec_scheme .gt. 6 ) then
            ...
            call error_mesg ( 'arm2003_data_read in scm_arm2003_mod',         &
...
!
    ...
    if( file_exist(trim(fname_nc)) ) then
          ...
          call mpp_error ('scm_arm2003_mod',                            &
    ...
    else if (file_exist('INPUT/arm2003_forc.res')) then

          unit=open_restart_file('INPUT/arm2003_forc.res',action='read')
...
end subroutine arm2003_data_read
...
subroutine arm2003_forc_init(time_interp,pdamp,As,Bs)
#include "fv_arrays.h"
...
!-----------------------------------------------------------------------
 ...
 end subroutine arm2003_forc_init

!#######################################################################
!#######################################################################


subroutine arm2003_forc_end ()

  integer :: unit
  character(len=64) :: fname_nc='RESTART/arm2003_forc.res.nc'
  
  if (.NOT.arm2003_forc_initialized) return
  arm2003_forc_initialized = .FALSE.
...
! write to the restart file
  ...
  if( do_netcdf_restart ) then
    ...
    if (mpp_pe() == mpp_root_pe()) then
       ...
       unit=open_restart_file('RESTART/arm2003_forc.res',action='write')
...
end subroutine arm2003_forc_end


!#######################################################################
!#######################################################################

subroutine arm2003_forc_diagnostic_init(axes, Time)
...
end subroutine arm2003_forc_diagnostic_init
...
subroutine update_arm2003_forc(time_interp,time_diag,dt_int,pdamp )
#include "fv_arrays.h"
...
if (time_interp < time_layer(1)) then
    ...
    call error_mesg ( 'update_arm2003_forc in scm_arm2003_mod',                &
...
end subroutine update_arm2003_forc
...
subroutine arm2003_surface_flux_loop (                                     &
...
end subroutine arm2003_surface_flux_loop

!#######################################################################
!########################################################################
! This subroutine returns surface conditions

subroutine get_arm2003_sfc( albedo_obs_out, tskin_out,     &
...
end subroutine get_arm2003_sfc

!########################################################################
!########################################################################


end module scm_arm2003_mod
