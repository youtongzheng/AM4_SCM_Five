module scm_Hawaii_143W_8N_mod
 
 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!       Hawaii_143W_8N (Oct.-Nov., 2008) FORCING MODULE
!
!       Dec 2011
...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public Hawaii_143W_8N_data_read, Hawaii_143W_8N_forc_init, Hawaii_143W_8N_forc_end, update_Hawaii_143W_8N_forc, &
          Hawaii_143W_8N_forc_diagnostic_init,        get_Hawaii_143W_8N_flx
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!        The module contains the following subroutines:
!
!            Hawaii_143W_8N_data_read      reads forcing data files and initializes
!                                 any needed parameters of the run
!            Hawaii_143W_8N_forc_init      initializes prognostic variables:           
!                                 T,u,v,qv,ql,qi
!            Hawaii_143W_8N_forc_end       deallocates allocated spaces
!            update_Hawaii_143W_8N_forc    a call to this routine returns the 
...
character(len=14) :: mod_name = 'Hawaii_143W_8N'
...
character*64                   :: fname_forc='INPUT/19810101.atmos_col_001.nc'

LOGICAL, PRIVATE               :: Hawaii_143W_8N_forc_initialized = .FALSE.
...
!--------------------- version number ----------------------------------
!
character(len=128) :: Version = '$Id: Hawaii_143W_8N.F90,v 1.1.2.1 2014/02/10 18:17:22 wfc Exp $'
character(len=128) :: Tagname = '$Name: new_gate_wfc $'
integer, dimension(1) :: restart_versions = (/ 1 /)
        
NAMELIST /scm_Hawaii_143W_8N_nml/   fname_forc, forc_begin_date,  dt_forc,                      &
...
subroutine Hawaii_143W_8N_data_read( )
...
integer                        :: i_forc_time
       ...
       ! set Hawaii_143W_8N_forc_initialized .TRUE.

       if ( Hawaii_143W_8N_forc_initialized ) return
       Hawaii_143W_8N_forc_initialized = .TRUE.

!    --------- read namelist --------
       if (file_exist('input.nml')) then
            unit = open_namelist_file()
            ierr=1; do while (ierr /= 0)
            read  (unit, nml=scm_Hawaii_143W_8N_nml, iostat=io, end=10)
            ierr = check_nml_error(io,'scm_Hawaii_143W_8N_nml')
            enddo
10          call close_file (unit)
 ...
 !--------- write version number and namelist --------
       ...
       if(mpp_pe() == mpp_root_pe() ) write(stdlog(),nml=scm_Hawaii_143W_8N_nml)


       if ( file_exist( fname_forc ) ) then
          if(mpp_pe() == mpp_root_pe() ) call mpp_error ('scm_Hawaii_143W_8N_mod', &
...
!      
       ...
       endif     !file_exist('INPUT/*.atmos_col_001.nc' )
!-----------------------------------------------------------------------
                            
end subroutine Hawaii_143W_8N_data_read


!#######################################################################
subroutine Hawaii_143W_8N_forc_init(time_interp, As, Bs)
#include "fv_arrays.h"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
! --- Create half pressure levels
      ...
      field_int_more(1:n_forc_levels) = qv_profile(1,1, 1:n_forc_levels, itime_more) &
       ...
       enddo
!-----------------------------------------------------------------------
 end subroutine Hawaii_143W_8N_forc_init



!#######################################################################
subroutine Hawaii_143W_8N_forc_end()

  if (.NOT.Hawaii_143W_8N_forc_initialized) return
  Hawaii_143W_8N_forc_initialized = .FALSE.

end subroutine Hawaii_143W_8N_forc_end

!#######################################################################


subroutine Hawaii_143W_8N_forc_diagnostic_init(axes, Time)
...
!-----------------------------------------------------------------------
end subroutine Hawaii_143W_8N_forc_diagnostic_init


!#######################################################################
subroutine update_Hawaii_143W_8N_forc(time_interp,time_diag,dt_int)
#include "fv_arrays.h"
...
! the surface fluxes are called before update_Hawaii_143W_8N_forc, 
! so the next time index is calculated for the surface fluxes at next time step!
       ...
       print*, ' update_forc, itime_more = ', itime_more,  weight_more
!-----------------------------------------------------------------------
end subroutine update_Hawaii_143W_8N_forc
...
subroutine get_Hawaii_143W_8N_flx( ustar, flux_t, flux_q )
...
end subroutine get_Hawaii_143W_8N_flx
!-----------------------------------------------------------------------
...
end module scm_Hawaii_143W_8N_mod
