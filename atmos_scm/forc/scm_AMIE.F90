...
!
!       WFC: 2015/05/27, modified to run for AMIE SCM.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public AMIE_data_read, AMIE_forc_init, AMIE_forc_end, update_AMIE_forc, &
          AMIE_forc_diagnostic_init,                      &
...
!
!            AMIE_data_read     reads forcing data files and initializes
!                              any needed parameters of the run
!            AMIE_forc_init     initializes prognostic variables:           
!                              T,u,v,qv,ql,qi
!            AMIE_forc_end      deallocates allocated spaces
!            update_AMIE_forc   a call to this routine returns the 
...
character(len=128) :: Version = '$Id: scm_AMIE.F90,v 1.1.2.1.2.2 2014/08/18 15:16:54 wfc Exp $'
character(len=128) :: Tagname = '$Name: a1_closure_wfc $'
        

NAMELIST /SCM_AMIE_NML/  do_netcdf_restart,                             &
                        ...
                        use_given_omega_surface, sphum_multiplier

LOGICAL :: AMIE_forc_initialized = .FALSE.
...
CHARACTER(len=64)                         :: fname_nc='INPUT/AMIE_forc.res.nc'
...
!      ----- read namelist -----
      ...
      ! set AMIE_forc_initialized .TRUE.

      if (AMIE_forc_initialized) return
      AMIE_forc_initialized = .TRUE.


      if (file_exist('input.nml')) then
#ifdef INTERNAL_FILE_NML
        read (input_nml_file, nml=SCM_AMIE_NML, iostat=io)
        ierr = check_nml_error(io, 'SCM_AMIE_NML')
#else
        unit = open_namelist_file ()
        io=1; do while (io .ne. 0)
          read  (unit, nml=SCM_AMIE_NML, iostat=io, end=10)
        enddo
...
!--------- write version and namelist to standard log ------------
        ...
        if ( mpp_pe() == mpp_root_pe() ) &
          write ( logunit, nml=SCM_AMIE_NML )
      ...
      !dummy check -------         
       if (temp_vert_advec_scheme .gt. 6 ) then
            ...
            call error_mesg ( 'AMIE_data_read in scm_AMIE_mod',         &
       ...
       if( file_exist(trim(forcing_nc)) ) then
         ...
         ivert= siz(1)
         ! For AMIE, assume the number of fields is 43.
         nfields = 43
         ! For AMIE, we assume the number of layer fields is 18 (No arscl variable).
         nlayer_fields = 18
         ASCII_VARA = .FALSE.
       endif
...
!They are converted to K later.
         ...
         enddo
!--- YLL, let tskin_forc = TSKIN
! tground = -9999 in data set         tskin_forc(:)=tground(:)
         tskin_forc(:)=tair_surf_forc(:)
!wfc
! Should the TSkin forcing be the ground temperature? No. Requested to be 29C = TSKIN = 302.15
...
!
    ...
    else if (file_exist('INPUT/AMIE_forc.res')) then

          unit=open_restart_file('INPUT/AMIE_forc.res',action='read')
...
subroutine AMIE_forc_init(time_interp, pdamp, As, Bs)
#include "fv_arrays.h"
...
!-----------------------------------------------------------------------
 ...
 end subroutine AMIE_forc_init

!#######################################################################
!#######################################################################


subroutine AMIE_forc_end ()

  integer :: unit
  character(len=64) :: fname_nc='RESTART/AMIE_forc.res.nc'
  
  if (.NOT.AMIE_forc_initialized) return
  AMIE_forc_initialized = .FALSE.
...
! write to the restart file
  ...
  if( do_netcdf_restart ) then
    ...
    if (mpp_pe() == mpp_root_pe()) then
       ...
       unit=open_restart_file('RESTART/AMIE_forc.res',action='write')
...
end subroutine AMIE_forc_end


!#######################################################################
!#######################################################################

subroutine AMIE_forc_diagnostic_init(axes, Time)
...
end subroutine AMIE_forc_diagnostic_init
...
subroutine update_AMIE_forc(time_interp,time_diag,dt_int,pdamp )
#include "fv_arrays.h"
...
if (time_interp < time_layer(1)) then
    ...
    call error_mesg ( 'update_AMIE_forc in scm_AMIE_mod',                &
...
end subroutine update_AMIE_forc
...
subroutine AMIE_surface_flux_loop (                                     &
...
end subroutine AMIE_surface_flux_loop

!#######################################################################
!########################################################################
! This subroutine returns surface conditions

subroutine get_AMIE_sfc( albedo_obs_out, tskin_out,     &
...
end subroutine get_AMIE_sfc

!########################################################################
! This subroutine returns imposed SST

subroutine get_AMIE_sst( sst )
