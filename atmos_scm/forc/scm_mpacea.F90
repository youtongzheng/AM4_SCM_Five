...
!
!       This module is modified from the one used for running data at 
!       the SGP so that it can run with data for the ARM North Slope of 
!       Alaska from the MPACE experiment (4-22 October, 2004).
!
!       References
!
...
!
   ...
   use      constants_mod, only:  rdgas, cp_air, tfreeze, hlv, rvgas,  &
                                  grav, stefan
...
use             fv_pack, only: nlon, mlat, nlev, beglat, endlat, beglon, &
   ...
   public mpacea_data_read, mpacea_forc_init, mpacea_forc_end, update_mpacea_forc, &
          SCM_MPACEA_NML,  mpacea_forc_diagnostic_init,                            &
...
!
!            mpacea_data_read     reads forcing data files and initializes
!                                 any needed parameters of the run
!            mpacea_forc_init     initializes prognostic variables:           
!                                 T,u,v,qv,ql,qi
!            mpacea_forc_end      deallocates allocated spaces
!            update_mpacea_forc   a call to this routine returns the 
...
!       rough_mom    momentum roughness length characteristic for MPACE
!	             region
!       rough_heat   heat roughness length = rough_mom
...
real,    public                :: rough_mom = 0.001
real,    public                :: rough_heat = 0.001
! Garratt is the reference for these roughness lengths
...
namelist /SCM_MPACEA_NML/ do_netcdf_restart,                           &
...
logical :: mpacea_forc_initialized = .false.
...
character(len=64)                         :: fname_nc='INPUT/scm_mpacea.res.nc'
...
!      ----- read namelist -----
      ...
      ! set mpacea_forc_initialized .TRUE.

      if (mpacea_forc_initialized) return
      mpacea_forc_initialized = .true.
#ifdef INTERNAL_FILE_NML
      read (input_nml_file, nml=SCM_MPACEA_NML, iostat=io)
      ierr = check_nml_error(io, 'SCM_MPACEA_NML')
#else
      if (file_exist('input.nml')) then
         unit = open_namelist_file ()
         io=1; do while (io .ne. 0)
            read  (unit, nml=SCM_MPACEA_NML, iostat=io, end=10)
         enddo
  10     call close_file (unit)
      ...
      !dummy check -------         
       if (temp_vert_advec_scheme .gt. 6 ) then
            ...
            call error_mesg ( 'mpacea_data_read in scm_mpacea_mod',         &
...
!
    ...
    else if (file_exist('INPUT/scm_mpacea.res')) then

          unit=open_restart_file('INPUT/scm_mpacea.res',action='read')
...
end subroutine mpacea_data_read
...
subroutine mpacea_forc_init(time_interp,pdamp,As,Bs)
#include "fv_arrays.h"
...
!      storage fields to a given time an pressure, to do the resulting
!      calculation.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
integer  :: itime_less, itime_more, KDIM, k, jstart, jstart_stand, jend
...
real, dimension(size(pt,1),size(pt,2),size(pt,3))   :: pi_fac, pfull
...
! --- compute ending level for variational dataset -----------------!
      jend = ivert
      do while(plev_forc(jend).lt.pdamp)
          jend=jend-1
      enddo
                          
! --- compute starting level for ECMWF reference profile  ------ !

      jstart_stand = 1
      do while (pdamp.lt.plev_stand(jstart_stand))
...
! --- CREATE INITIAL pt  ---------------------------------------------- !
       ...
       field_int_more(1)=t_forc(jstart,itime_more)*                    &
            ( (  (ps_init(1,1)/plev_forc(jstart)) )**(rdgas/cp_air) )
       plev_int(2:(jend-jstart+2))=plev_forc(jstart:jend)
       field_int_less(2:(jend-jstart+2))=                             &
                                   t_forc(jstart:jend,itime_less)
       field_int_more(2:(jend-jstart+2))=                             &
                                   t_forc(jstart:jend,itime_more)
       
       plev_int((jend-jstart+3):                                      &
                (jend-jstart+2+ivert_stand-jstart_stand+1))=                    &
            plev_stand(jstart_stand:ivert_stand)
       field_int_less((jend-jstart+3):                                &
                (jend-jstart+2+ivert_stand-jstart_stand+1))=          &
                       t_stand(jstart_stand:ivert_stand)
       field_int_more((jend-jstart+3):                                &
                (jend-jstart+2+ivert_stand-jstart_stand+1))=          &
                       t_stand(jstart_stand:ivert_stand)
                       
       nlev_interp = jend-jstart+2+ivert_stand-jstart_stand+1
       tmp_p(:)=pfull(1,1,:)
...
! --- CREATE INITIAL Qv  --------------------------------------------- !
       ...
       ! --- find nearest time to time_interp in Layered fields ------ !
       ! --- no need since this was done for pt 
       ...
       plev_int(2:(jend-jstart+2))=plev_forc(jstart:jend)
       field_int_less(2:(jend-jstart+2))=                             &
                                   q_forc(jstart:jend,itime_less)
       field_int_more(2:(jend-jstart+2))=                             &
                                   q_forc(jstart:jend,itime_more)
       ...
       field_int_less((jend-jstart+3):                                &
                (jend-jstart+2+ivert_stand-jstart_stand+1))=          &
                       q_stand(jstart_stand:ivert_stand)
       field_int_more((jend-jstart+3):                                &
                (jend-jstart+2+ivert_stand-jstart_stand+1))=          &
                       q_stand(jstart_stand:ivert_stand)
...
!-----------------------------------------------------------------------
 ...
 end subroutine mpacea_forc_init

!#######################################################################
!#######################################################################


subroutine mpacea_forc_end ()

  integer :: unit
  character(len=64) :: fname_nc='RESTART/scm_mpacea.res.nc'
  
  if (.not.mpacea_forc_initialized) return
  mpacea_forc_initialized = .false.
...
! write to the restart file
  ...
  if( do_netcdf_restart ) then
    ...
    if (mpp_pe() == mpp_root_pe()) then
       ...
       unit=open_restart_file('RESTART/scm_mpacea.res',action='write')
...
end subroutine mpacea_forc_end


!#######################################################################
!#######################################################################

subroutine mpacea_forc_diagnostic_init(axes, Time)
...
end subroutine mpacea_forc_diagnostic_init
...
subroutine update_mpacea_forc(time_interp,time_diag,dt_int,pdamp)
#include "fv_arrays.h"
...
if (time_interp < time_layer(1)) then
    ...
    call error_mesg ( 'update_mpacea_forc in scm_mpacea_mod',          &
...
case("F")
  ...
  !---- COMPUTE FINAL ADVECTIVE pt TENDENCY FROM THE SUM OF :  ---------!
...
case default
      ...
      !---------- Create Zero Forcing for p < pdamp ---------------!
      do k = 1, KDIM
           where (pfull(:,:,k).lt. pdamp)
                t_dt(:,:,k) = 0.
                nudg_t(:,:,k) = 0.
           end where     
      enddo
...
! ----------------- PRODUCE CONDENSATE FORCING ------------------------------ !
      ...
      if (vert_advec_cond) then
          ...
          endif !  present(qn) .and. present(AN)
          ...
          endif ! present(qni) .and. present(ANI)
...
end subroutine update_mpacea_forc
...
subroutine mpacea_surface_flux_loop (                                  &
...
end subroutine mpacea_surface_flux_loop

!#######################################################################
!########################################################################
! This subroutine returns surface conditions

subroutine get_mpacea_sfc( albedo_obs_out, tskin_out,     &
...
end subroutine get_mpacea_sfc

!########################################################################
!########################################################################


end module scm_mpacea_mod
