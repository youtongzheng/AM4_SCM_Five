...
! --- FMS ----
   ...
   use mpp_mod, only: mpp_pe, mpp_root_pe, mpp_npes, MPP_VERBOSE, &
        mpp_get_current_pelist, mpp_sync, mpp_error, input_nml_file, FATAl
...
#ifdef SCM
!  The following are default resolutions for the SCM.
!  The actual resolution will be defined within the grid-spec file and will
!   be read in atmosphere_init
   real :: SCM_RES_X = 2.5 ! longitudinal resolution (degrees)
   real :: SCM_RES_Y = 2.5 ! latitudinal resolution (degrees)
   real :: target_lon  = 0.0 ! Target longitude (degrees) for use with SCM
   real :: target_lat  = 0.0 ! Target latitude (degrees) for use with SCM
#endif
   ...
   logical :: esm2_bugs = .false.             ! needed to ensure a bug in compute_g_avg
                                              ! is propogated when .TRUE.
...
#endif MARS_GCM 
                         layout, fv_pset_stack, esm2_bugs
#else
   namelist /fv_core_nml/nlon,      mlat,     nlev,                           &
                         ncnst,     nt_phys,  pnats,                          &
                         iord_mt,   iord_tm,  iord_tr,                        &
...
!PUBLIC VARIABLES
   public :: nlon, mlat, nlev, beglon, endlon, beglat, endlat, &
        ...
        change_time, grid_weight, esm2_bugs
#ifdef USE_LIMA
...
#endif
#ifdef SCM
   public ::  target_lon, target_lat, SCM_RES_X, SCM_RES_Y
#endif SCM 
...
!
    if (use_tendency) then
      call mpp_error (FATAL, 'The use_tendency option has been removed as of the Ulm release.')
    endif
...
! This routine should only be called once per run
   ...
   pi = 4.d0 * atan(1.d0)
#ifdef SCM
   dl = SCM_RES_X*pi/180. !Arbitrary SCM_RES_X degrees
   dp = SCM_RES_Y*pi/180. !Arbitrary SCM_RES_Y degrees
   ph5 = target_lat*pi/180.
   sine(1) = sin(ph5 - 0.5*dp) ! sin(Southern Boundary)
   sine(2) = sin(ph5 + 0.5*dp) ! sin(Northern Boundary)

   grid_weight(1) = 0.5!sine(2) - sine(1)
   grid_weight(2) = 0.5!sine(2) - sine(1)
...
! Define cosine at edges..
   ...
   cose(1) = cos(ph5 - 0.5*dp)
   cose(2) = cos(ph5 + 0.5*dp)

   acosu(1) = 2. / (cose(1) + cose(2))
   acosu(2) = acosu(1)

   sinp(1) = 0.5 * (sine(1) + sine(2))
   sinp(2) = sinp(1)
...
!-------------
      ...
      latb(1) = target_lat - 0.5*SCM_RES_Y ! Southern boundary (degrees)
      latb(2) = target_lat + 0.5*SCM_RES_Y ! Northern boundary (degrees)
      lat(1) = target_lat ! Box Center (degrees)
...
!-------------------------------
      ...
      do i=1,nlon+1
#ifdef SCM
         lonb(i) = target_lon - 0.5*SCM_RES_X + SCM_RES_X*(i-1) ! Western and Eastern boundaries (degrees)
