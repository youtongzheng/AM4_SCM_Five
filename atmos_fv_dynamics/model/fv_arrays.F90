...
#ifdef SCM
! Avoid the need for 3 latitude bands.
#else
...
! For GFDL physics:
        allocate (ps_bp (isg:ieg, js:je))        ; ps_bp(isg:ieg, js:je) = 0.0
        allocate ( u_srf(isg:ieg, js:je) )       ; u_srf(isg:ieg, js:je) = 0.0
        allocate ( v_srf(isg:ieg, js:je) )       ; v_srf(isg:ieg, js:je) = 0.0
        allocate( u_dt(isg:ieg, js:je, nz) )     ; u_dt(isg:ieg, js:je, :)    = 0.0
        allocate( v_dt(isg:ieg, js:je, nz) )     ; v_dt(isg:ieg, js:je, :)    = 0.0
        allocate( t_dt(isg:ieg, js:je, nz) )     ; t_dt(isg:ieg, js:je, :)    = 0.0
        allocate( q_dt(isg:ieg, js:je, nz, nt) ) ; q_dt(isg:ieg, js:je, :, :) = 0.0
#ifdef MARS_GCM
